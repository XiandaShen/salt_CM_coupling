...
checkdts=cell(timesteps,1); % check the change of total stress
for i=1:timesteps
    checkdts(i)={zeros(3,3)};
end
rcheckdts=cell(timesteps,1); % recheck the change of total stress
for i=1:timesteps
    rcheckdts(i)={zeros(3,3)};
end
...
% thick=0.01*ones(1,cellnumber); % thickness of each cell, can be used to calculate the iner abc, it should not be larger than cell size
 ...
 cella=0.29:0.01:(0.01*(cellnumber-1)+0.29); 
 cellb=0.28999:0.01:(0.01*(cellnumber-1)+0.28999);
 cellc=0.28998:0.01:(0.01*(cellnumber-1)+0.28998);
 thick=0.01*ones(1,cellnumber); % thickness of each cell, can be used to calculate the iner abc, it should not be larger than cell size

%  cella=0.29*ones(1,cellnumber); 
%  cellb=0.289999*ones(1,cellnumber);
%  cellc=0.289998*ones(1,cellnumber);
%  thick=0.01*ones(1,cellnumber); % thickness of each cell, can be used to calculate the iner abc, it should not be larger than cell size
...
checkdts(j+1)={checkdts{j+1}+dsigma{j+1,cn}};

end

for cn=1:cellnumber
   dsigma(j+1,cn)={dsigma{j+1,cn}-checkdts{j+1}/cellnumber};  
   sigma(j+1,cn)={sigma{j,cn}+dsigma{j+1,cn}};
end

for cn=1:cellnumber
   rcheckdts(j+1)={rcheckdts{j+1}+dsigma{j+1,cn}};  
end
