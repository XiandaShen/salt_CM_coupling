...
timesteps=4000; % creep time steps
...
% for i=1:cellnumber
% % phi(i)=mod(i,5);
% theta(i)=pi/2/cellnumber*fix((i-1)/cellnumber*5);
% 
% 
% end
...
F0=zeros(6*cellnumber+5,1); % new equations vector error
x0=zeros(6*cellnumber+5,1); % new dsigma iteration vector error
F1=zeros(6*cellnumber+5,1); % current equations vector error 
x1=zeros(6*cellnumber+5,1); % current dsigma iteration vector error
xnew=zeros(6*cellnumber+5,1); % current dsigma iteration vector error
invBFGS=eye(6*cellnumber+5);
...
dinter(1,:)=dsigma(j,:);
storex1=zeros(6*cellnumber+5,totaliter);
storeF1=zeros(6*cellnumber+5,totaliter);
arfak=1;
iteralpha=0;
...
% chemomechanics
 [F1,totalp,tepsilon,dinter,dsigma,sigma,depsilon,epsilon,epsilone,cellr,thick,dvcepsilon]=ncalibrationchemomechanics(creepinterval,j,itnum,cellnumber,cellr,Q,thick,dHdL,dsigma,dtsigma,sigma,depsilon,epsilon,tepsilon,epsilone,depsilone,dvcepsilon,totalp,dinter,x1,muo);
...
% 
% if iteralpha>10000 || norm(x1,'inf')>500
% 
% disa=disa+0.05
%     x1=disa*orgx1;
% 
%     iteralpha=0;
%     invBFGS=orginvBFGS;
%      [F1]=discalibrationchemomechanics(creepinterval,j,itnum,cellnumber,cellr,Q,thick,dHdL,dsigma,dtsigma,sigma,depsilon,epsilon,tepsilon,epsilone,depsilone,dvcepsilon,totalp,dinter,x1,muo);
% testdis=1;
% pk=-invBFGS*F1;
% end
...
%[Fnew,totalp,tepsilon,dinter,dsigma,depsilon,epsilon,cella,cellb,cellc,thick]=puremechanics(j,itnum,cellnumber,cella,cellb,cellc,thick,dsigma,dtsigma,depsilon,epsilon,tepsilon,totalp,dinter,xnew);
 % chemomechanics
 [Fnew,totalp,tepsilon,dinter,dsigma,sigma,depsilon,epsilon,epsilone,cellr,thick,dvcepsilon]=ncalibrationchemomechanics(creepinterval,j,itnum,cellnumber,cellr,Q,thick,dHdL,dsigma,dtsigma,sigma,depsilon,epsilon,tepsilon,epsilone,depsilone,dvcepsilon,totalp,dinter,xnew,muo);
