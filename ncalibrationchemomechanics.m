function [FF1,totalp,tepsilon,dinter,dsigma,sigma,depsilon,epsilon,epsilone,cellr,thick,dvcepsilon]=ncalibrationchemomechanics(creepinterval,j,itnum,cellnumber,cellr,Q,thick,dHdL,dsigma,dtsigma,sigma,depsilon,epsilon,tepsilon,epsilone,depsilone,dvcepsilon,totalp,dinter,xx1,muo)
...
FF1=zeros(6*cellnumber+5,1); % current equations vector error 
% dr=zeros(1,cellnumber); 
dfi=zeros(1,cellnumber);

for putback=1:cellnumber   
    dinter(itnum,putback)={[xx1(6*(putback-1)+1) xx1(6*(putback-1)+2) xx1(6*(putback-1)+3);xx1(6*(putback-1)+2) xx1(6*(putback-1)+4) xx1(6*(putback-1)+5);xx1(6*(putback-1)+3) xx1(6*(putback-1)+5) xx1(6*(putback-1)+6)]};
   dsigma(j,putback)=dinter(itnum,putback); 
       sigma(j+1,putback)={sigma{j,putback}+dsigma{j,putback}};
end  

dtsigma(1,1)=xx1(cellnumber*6+1);
dtsigma(1,2)=xx1(cellnumber*6+2);
dtsigma(1,3)=xx1(cellnumber*6+3);
dtsigma(2,2)=xx1(cellnumber*6+4);
dtsigma(2,3)=xx1(cellnumber*6+5);
...
for cn=1:cellnumber
 ...
 [thick(j+1,cn),vce,dfi(cn)]=chemocell(sigma{j+1,cn},cellr(cn),Q{cn},thick(j,cn),creepinterval);
...
for cn=1:cellnumber
    ...
    Fcn=-dinter{itnum,cn}+dtsigma-doubledotft(dcellH,(epsilon{j+1,cn}-tepsilon))-doubledotft(cellH{cn},(depsilon{j+1,cn}-dtepsilon));
...
% modified
    ...
    FF1(6*cellnumber+1)=tepsilon(1,1);
    FF1(6*cellnumber+2)=tepsilon(1,2);
    FF1(6*cellnumber+3)=tepsilon(1,3);
    FF1(6*cellnumber+4)=tepsilon(2,2);
    FF1(6*cellnumber+5)=tepsilon(2,3);
