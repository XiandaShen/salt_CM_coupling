function [FF1]=discalibrationchemomechanics(creepinterval,j,itnum,cellnumber,cellr,Q,thick,dHdL,dsigma,dtsigma,sigma,depsilon,epsilon,tepsilon,epsilone,depsilone,dvcepsilon,totalp,dinter,xx1,muo)
...
depsilone(j+1,cn)={doubledotft(inversegeneral(cellL{cn}),(dsigma{j,cn}-doubledotft((cellL{cn}-cellLF{cn}),epsilone{j,cn})))};
...
for cn=1:cellnumber
 ...
 % modified
    FF1(6*(cn-1)+1)=tepsilon(1,1);
    FF1(6*(cn-1)+2)=tepsilon(1,2);
    FF1(6*(cn-1)+3)=tepsilon(1,3);
    FF1(6*(cn-1)+4)=tepsilon(2,2);
    FF1(6*(cn-1)+5)=tepsilon(2,3);
